...
def main():
    ...
    original=np.array(Image.open(OUT.parent/'chest_breath_v4/calibration/after_3.png'))
    ...
    outside[710:835,220:520]=0
    assert outside.max()<=1,float(outside.max())
    assert abs(upper['dy_px']+original_upper['dy_px'])<=1
    assert abs(lower['dy_px']+original_lower['dy_px'])<=1
    report={'scope':'ParamBreath 0 to 1, fixed head at 1080px preview scale; not actual narration amplitude.',
            ...
            'original_v4_upper_fold_motion_vs_v3':original_upper,
            'original_v4_lower_fold_motion_vs_v3':original_lower,
            'outside_knit_regions_max_difference_v4_v5':float(outside.max()),
    ...
    assert upper['dy_px']<0 and lower['dy_px']>0,report
