"""Synchronized A/B view: identical scale and time; only knit vertical motion differs."""
...
def main():
    old=EXPORTS/'Risa_ModelOnly_BreathOnly_v4.mp4'
    new=EXPORTS/'Risa_ModelOnly_BreathOnly_v5.mp4'
    target=EXPORTS/'Risa_Breath_Direction_Compare_v4_v5.mp4'
    header=Image.new('RGB',(1920,80),(24,32,44))
    draw=ImageDraw.Draw(header);font=ImageFont.load_default(size=34)
    draw.text((30,22),'A: v4 ORIGINAL',font=font,fill='white')
    draw.text((990,22),'B: v5 VERTICAL REVERSED',font=font,fill='white')
    header.save(OUT/'comparison_header.png')
    ffmpeg(['-i',old,'-i',new,'-loop','1','-framerate','30','-i',OUT/'comparison_header.png',
            '-filter_complex','[0:v]scale=960:960,setpts=PTS-STARTPTS[a];[1:v]scale=960:960,setpts=PTS-STARTPTS[b];[a][b]hstack=inputs=2[body];[2:v][body]vstack=inputs=2[v]',
    ...
    for index in [0,60,120,240]:
        ...
        errors[str(index)]=[]
        for j,folder in enumerate([OUT.parent/'chest_breath_v4',OUT]):
            source=Image.open(folder/f'breath_only/{index:05d}.png').resize((960,960),Image.Resampling.LANCZOS)
    ...
    assert sha(OUT/'breath_only.motion3.json')==sha(OUT.parent/'chest_breath_v4/breath_only.motion3.json')
    write(OUT/'comparison_verification.json',{'passed':True,'output':str(target),'sha256':sha(target),
        'layout':'Left A=v4 original; right B=v5 vertical reversal. Same 960x960 scale and same frames.',
