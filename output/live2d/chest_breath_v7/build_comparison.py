"""Synchronized A/B view: v6 subtle inward folds vs v7 internal convergence."""
...
def main():
    old=EXPORTS/'Risa_ModelOnly_BreathOnly_v6.mp4'
    new=EXPORTS/'Risa_ModelOnly_BreathOnly_v7.mp4'
    target=EXPORTS/'Risa_Throat_Folds_Compare_v6_v7.mp4'
    header=Image.new('RGB',(1920,80),(24,32,44))
    draw=ImageDraw.Draw(header);font=ImageFont.load_default(size=34)
    draw.text((30,22),'A: v6 INWARD FOLDS',font=font,fill='white')
    draw.text((990,22),'B: v7 STRONGER FOLDS',font=font,fill='white')
    ...
    for index in [0,60,120,240]:
        ...
        errors[str(index)]=[]
        for j,folder in enumerate([OUT.parent/'chest_breath_v6',OUT]):
    ...
    assert sha(OUT/'breath_only.motion3.json')==sha(OUT.parent/'chest_breath_v6/breath_only.motion3.json')
    write(OUT/'comparison_verification.json',{'passed':True,'output':str(target),'sha256':sha(target),
        'layout':'Left A=v6 subtle inward folds; right B=v7 stronger inward folds. Same crop (270,310,810,850), enlarged to 960x960; same frames.',
