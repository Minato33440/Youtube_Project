...
def main():
    ...
    original=np.array(Image.open(OUT.parent/'chest_breath_v6/calibration/after_3.png'))
    ...
    report={'scope':'ParamBreath 0 to 1, fixed head at 1080px preview scale; not actual narration amplitude.',
            ...
            'original_v6_upper_fold_motion_vs_v3':original_upper,
            'original_v6_lower_fold_motion_vs_v3':original_lower,
            'original_v6_left_fold':original_left,
            'original_v6_right_fold':original_right,
            'left_internal_fold_vs_v3':left_fold,
            'right_internal_fold_vs_v3':right_fold,
            'outside_upper_knit_max_difference_v6_v7':float(outside.max()),
            'abdomen_max_difference_v6_v7':float(pair_delta[700:].max()),
            'silhouette_alpha_max_difference_v6_v7':int(np.abs(original[:,:,3].astype(int)-b[:,:,3].astype(int)).max()),
    ...
    assert left_fold['dx_px']>=original_left['dx_px']+2,report
    assert right_fold['dx_px']<=original_right['dx_px']-2,report
    assert report['abdomen_max_difference_v6_v7']==0,report
    ...
    assert report['silhouette_alpha_max_difference_v6_v7']<=1,report
