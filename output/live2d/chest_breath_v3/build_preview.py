...
OLD_RIG=OUT.parent/'chest_breath_v2/Risa_chest_breath.model3.json'
...
def calibration():
    ...
    for i in range(4):
        ...
        fixed.append({'breath':cases[i]['breath'],
                      ...
                      'central_collar_mean_difference':float(d[350:420,350:420].mean()),
                      'central_collar_max_difference':int(d[350:420,350:420].max())})
    assert all(m['head_max_difference']<=1 for m in fixed),fixed
    assert all(m['central_collar_mean_difference']<1 for m in fixed),fixed
    assert metrics[3]['mean_chest_pixel_difference']>1,metrics
    write(OUT/'calibration_verification.json',{'passed':True,'cases':metrics,'fixed_neck_checks':fixed,'scope':'v2/v3 comparison is descriptive; acceptance checks compare v3 breath 0/.3/.6/1 at neutral head. Global breath scale intentionally removed.'})
...
def build():
    ...
    primary=EXPORTS/'Risa_ModelOnly_ChestBreath_v3.mp4'
    breathing=EXPORTS/'Risa_ModelOnly_BreathOnly_v3.mp4'
    ...
    write(OUT/'manifest.json',{'primary':str(primary),'breath_only':str(breathing),
        ...
        'breathing_geometry':'Shoulder-side controls rise 6-7 editor pixels; chest controls expand horizontally 6 pixels, lower chest 4 pixels; central collar and waist controls fixed; no chest vertical translation.',
