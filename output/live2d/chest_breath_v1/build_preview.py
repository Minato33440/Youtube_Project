...
OLD_RIG=OUT.parent/'voice_sync_v4/model/Risa_mouth_shapes.model3.json'
...
class Renderer:
    ...
    def frame(self,values,dt=1/FPS,motion=False):
        ...
        self.pg.event.pump();self.model._model.Update(dt)
        if not motion:
...
def calibration():
    ...
    assert all(m['head_max_pixel_difference']==0 for m in metrics),metrics
    assert all(m['new_interior_transparency_pixels']==0 for m in metrics),metrics
    assert metrics[3]['mean_chest_pixel_difference']>1,metrics
    write(OUT/'calibration_verification.json',{'passed':True,'cases':metrics,'scope':'Same parameters on old and new native rigs; isolates new chest deformation. Head region y<300; transparency compares eroded old model alpha.'})
...
def build():
    ...
    # local deformation is the only change to the speaking performance.
    ...
    primary=EXPORTS/'Risa_ModelOnly_ChestBreath_v1.mp4'
    breathing=EXPORTS/'Risa_ModelOnly_BreathOnly_v1.mp4'
    ...
    write(OUT/'manifest.json',{'primary':str(primary),'breath_only':str(breathing),
        'rig':'Chest_Breath (Warp4) under Breath_Base, containing only Body_Clothes; ParamBreath keys 0 and 1',
        'breathing_geometry':'At maximum inhalation, raise upper-chest controls and spread sides; top/collar and waist/lower grid points unchanged.',
