...
def main():
    ...
    original=np.array(Image.open(OUT.parent/'chest_breath_v5/calibration/after_3.png'))
    ...
    report={'scope':'ParamBreath 0 to 1, fixed head at 1080px preview scale; not actual narration amplitude.',
            ...
            'original_v5_upper_fold_motion_vs_v3':original_upper,
            'original_v5_lower_fold_motion_vs_v3':original_lower,
            'left_internal_fold_vs_v3':left_fold,
            'right_internal_fold_vs_v3':right_fold,
            'outside_upper_knit_max_difference_v5_v6':float(outside.max()),
            'abdomen_max_difference_v5_v6':float(pair_delta[700:].max()),
            'silhouette_alpha_max_difference_v5_v6':int(np.abs(original[:,:,3].astype(int)-b[:,:,3].astype(int)).max()),
    ...
    assert report['abdomen_max_difference_v5_v6']==0,report
    # The 27 changed alpha samples are near-opaque interior texels (252..255),
    # differing by 1 after interpolation. Test the visible silhouette separately.
    assert report['silhouette_alpha_max_difference_v5_v6']<=1,report
