...
def main():
    ...
    configs=[('speaking','Risa_ModelOnly_ChestBreath_v4.mp4',479,main_indices),
             ('breath_only','Risa_ModelOnly_BreathOnly_v4.mp4',241,[0,30,60,90,120,180,240])]
    result={}
    for label,name,count,indices in configs:
        ...
        if label=='speaking':
            ...
            # These coordinates include the model's 156px horizontal inset.
            delta=np.abs(sampled[0].astype(float)-sampled[2].astype(float))
            data['upper_knit_fold_temporal_mean_rgb_difference']=float(delta[400:510,476:606].mean())
            data['lower_knit_fold_temporal_mean_rgb_difference']=float(delta[730:815,436:656].mean())
            assert data['upper_knit_fold_temporal_mean_rgb_difference']>.3
            assert data['lower_knit_fold_temporal_mean_rgb_difference']>.3
        ...
        result[label]=data
    # Verify the fixed head separately from the intentionally moving knit folds.
    calibration=json.loads((OUT/'calibration_verification.json').read_text(encoding='utf-8'))
    local=json.loads((OUT/'local_motion_verification.json').read_text(encoding='utf-8'))
    assert calibration['passed']
    assert local['passed']
    face_errors=[v['head_max_difference'] for v in calibration['fixed_neck_checks']]
    assert max(face_errors)==0
    report={'passed':True,'previous_speech_head_blink_breath_curves_identical':True,
            'local_motion_verification':local,
            'fixed_head_breath_sweep_max_pixel_differences':face_errors,'fixed_neck_checks':calibration['fixed_neck_checks'],'outputs':result,
