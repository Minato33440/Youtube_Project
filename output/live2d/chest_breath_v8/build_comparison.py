"""Synchronized A/B view: v7 baseline vs v8 additional upper-collar crease motion."""
...
def main():
    old=EXPORTS/'Risa_ModelOnly_BreathOnly_v7.mp4'
    new=EXPORTS/'Risa_ModelOnly_BreathOnly_v8.mp4'
    target=EXPORTS/'Risa_Throat_Folds_Compare_v7_v8.mp4'
    header=Image.new('RGB',(1920,80),(24,32,44))
    draw=ImageDraw.Draw(header);font=ImageFont.load_default(size=34)
    draw.text((30,22),'A: v7 PREVIOUS',font=font,fill='white')
    draw.text((990,22),'B: v8 UPPER CREASE',font=font,fill='white')
    ...
    for index in [0,60,120,240]:
        ...
        errors[str(index)]=[]
        for j,folder in enumerate([OUT.parent/'chest_breath_v7',OUT]):
    ...
    assert sha(OUT/'breath_only.motion3.json')==sha(OUT.parent/'chest_breath_v7/breath_only.motion3.json')
    write(OUT/'comparison_verification.json',{'passed':True,'output':str(target),'sha256':sha(target),
        'layout':'Left A=v7 previous; right B=v8 added upper crease motion. Same crop (270,310,810,850), enlarged to 960x960; same frames.',
