...
def main():
    ...
    for name in ['Risa_narration_composite.mp4', 'Risa_lipsync_closeup.mp4', 'Risa_lipsync_comparison.mp4']:
        path = ROOT / name
        result = probe(path)
    ...
    settings = json.loads((ROOT / 'mouth_curve.json').read_text(encoding='utf-8'))
    anchors = settings['closure_anchors']
    assert all(mouth[a['frame']] == 0 for a in anchors)
    assert np.abs(np.diff(mouth)).max() <= .340001
    assert mouth[-1] == 0
    prior = np.array([float(r['v1_open']) for r in rows])
    report['phoneme_adjustments'] = {
        'bilabial_closure_anchors': [{**a, 'v1_open': float(prior[a['frame']]), 'v2_open': float(mouth[a['frame']])} for a in anchors],
        'max_step': float(np.abs(np.diff(mouth)).max()),
        'acoustic_silence_intervals': settings['acoustic_silence_intervals'],
    }
