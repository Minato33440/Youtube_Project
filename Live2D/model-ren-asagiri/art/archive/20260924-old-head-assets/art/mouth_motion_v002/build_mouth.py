...
def mouth(amount):
 t=float(amount);width_scale=.41+.25*t;sy=max(.001,.42*t)
 sx=(GX-CENTER_X)/width_scale+121
 u=np.clip(sx/242,0,1);closed_curve=1017+5*np.sin(np.pi*u)
 top_anchor=closed_curve-t*(22+5*np.sin(np.pi*u))
 ...
 ta=np.asarray(parts['mouth_teeth_upper']);white=(ta[:,:,:3].mean(2)>190)&(ta[:,:,3]>100)
 tooth_bottom=np.array([np.where(white[:,x])[0][-1] if white[:,x].any() else 25 for x in range(DONOR[0])],dtype='float32')
 tooth_bottom=cv2.GaussianBlur(tooth_bottom.reshape(1,-1),(21,1),5).ravel()
 end_lift=.52*np.clip(np.interp(sx,np.arange(DONOR[0]),tooth_bottom)-30,0,22)*(1-t)
 teeth=remap(parts['mouth_teeth_upper'],sx,(GY-top_anchor+12*(1-t)+end_lift)/.52)
 # Tongue follows the lower lip, with less vertical compression than the cavity.
 tongue_scale=.42*(.60+.40*t);bottom=top_anchor+sy*183
 tongue=remap(parts['mouth_tongue'],sx,(GY-bottom-35*(1-t))/tongue_scale+183)
 mask=np.asarray(cavity)[:,:,3]/255
 teeth=alpha_mul(teeth,mask);tongue=alpha_mul(tongue,mask)
 rims=[]
 for n in ['mouth_rim_upper','mouth_rim_lower']:
  ...
  stroke_scale=sy+(.52+.04*t-sy)*keep
...
states=[('closed',0),('small',.28),('medium',.55),('open',1)];stats={};renders=[]
for label,t in states:
 ...
 'output':{'psd':f'Ren_mouth_{label}_v002.psd','flattenedPng':'assembled.png','previewPng':'preview.png','verificationJson':'psd_verification.json','previewBackground':'#657078'}}
...
sheet=Image.new('RGB',(400*len(states),430),(101,112,120));d=ImageDraw.Draw(sheet)
...
face_sheet=Image.new('RGB',(380*len(states),600),(101,112,120))
...
frames=[];dur=[]
sequence=[(0,700),(.06,70),(.12,70),(.2,70),(.28,250),(.4,90),(.55,450),(.8,90),(1,650),(.8,90),(.55,200),(.4,90),(.28,150),(.2,70),(.12,70),(.06,70),(0,700)]
